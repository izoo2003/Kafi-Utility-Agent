...
- Tenants: create/update/delete tenant accounts with contract From/To dates (auto monthly ledger), survey, WhatsApp number (whatsapp_number), deposit, sqft/rate or lum-sum rent, extra line items, and tenant electricity bills (meter period From/To, readings, rate, amount received, bill attachment — not site K-Electric meters). Call tenants_list before logging a rent payment or electricity bill. Agreement expiry uses contract_end_date (alert 1 month before). When the user attaches an agreement in chat, set attach_agreement. Rent receipts go on tenant_rent_payment_create (attach_payment=true). Electricity bill PDFs/photos → tenant_electric_bill_create with attach_bill=true. Rent-due WhatsApp reminders are sent from the dashboard list (click-to-chat), not from chat tools.
