...
const sql = readFileSync(
  "supabase/migrations/20260903120000_solar_net_metering_per_site.sql",
  "utf8",
);
...
async function main() {
  if (databaseUrl) {
    ...
    try {
      ...
      console.log("Applied per-site net metering ledger via DATABASE_URL");
      await client.end();
      return;
    } catch (e) {
      if (/already exists|duplicate/i.test(String(e.message ?? e))) {
        console.log("Per-site net metering index already exists");
        await client.end();
        return;
      }
  ...
  for (const region of regions) {
    ...
    try {
      await client.connect();
      await client.query(sql);
      console.log("Applied per-site net metering ledger via", region);
      await client.end();
      return;
    } catch (e) {
      lastErr = e;
