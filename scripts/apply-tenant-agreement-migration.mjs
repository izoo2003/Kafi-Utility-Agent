...
const sql = readFileSync(
  "supabase/migrations/20260831160000_tenant_agreement_documents.sql",
  "utf8",
);
...
async function main() {
  if (databaseUrl) {
    ...
    try {
      ...
      console.log("Applied tenant agreement / documents via DATABASE_URL");
      await client.end();
      return;
    } catch (e) {
      if (/already exists|duplicate/i.test(String(e.message ?? e))) {
        console.log("Tenant agreement columns / bucket already exist");
        await client.end();
        return;
      }
  ...
  for (const region of regions) {
    ...
    try {
      await client.connect();
      await client.query(sql);
      console.log("Applied tenant agreement / documents via", region);
      await client.end();
      return;
    } catch (e) {
      lastErr = e;
