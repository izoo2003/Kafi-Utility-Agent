...
const sql = readFileSync(
  "supabase/migrations/20260831140000_water_tanker_drinking_water.sql",
  "utf8",
);
...
async function main() {
  if (databaseUrl) {
    ...
    try {
      ...
      console.log("Applied water tanker / drinking water seed via DATABASE_URL");
      await client.end();
      return;
    } catch (e) {
      if (/already exists|duplicate/i.test(String(e.message ?? e))) {
        console.log("Water tanker / drinking water accounts already exist");
        await client.end();
        return;
      }
  ...
  for (const region of regions) {
    ...
    try {
      await client.connect();
      await client.query(sql);
      console.log("Applied water tanker / drinking water seed via", region);
      await client.end();
      return;
    } catch (e) {
      lastErr = e;
