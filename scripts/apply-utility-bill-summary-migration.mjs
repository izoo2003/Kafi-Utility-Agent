...
const sql = readFileSync(
  "supabase/migrations/20260831150000_utility_bill_summary.sql",
  "utf8",
);
...
async function main() {
  if (databaseUrl) {
    ...
    try {
      ...
      console.log("Applied utility bill summary columns via DATABASE_URL");
      await client.end();
      return;
    } catch (e) {
      if (/already exists|duplicate/i.test(String(e.message ?? e))) {
        console.log("Utility bill summary columns already exist");
        await client.end();
        return;
      }
  ...
  for (const region of regions) {
    ...
    try {
      await client.connect();
      await client.query(sql);
      console.log("Applied utility bill summary columns via", region);
      await client.end();
      return;
    } catch (e) {
      lastErr = e;
