...
const sql = readFileSync(
  "supabase/migrations/20260831120000_solar_maintenance.sql",
  "utf8",
);
...
async function main() {
  if (databaseUrl) {
    ...
    try {
      ...
      console.log("Applied solar maintenance migration via DATABASE_URL");
      await client.end();
      return;
    } catch (e) {
      if (/already exists/i.test(String(e.message ?? e))) {
        console.log("solar_maintenance already exists");
        await client.end();
        return;
      }
  ...
  for (const region of regions) {
    ...
    try {
      await client.connect();
      await client.query(sql);
      console.log("Applied solar maintenance migration via", region);
      await client.end();
      return;
    } catch (e) {
      lastErr = e;
